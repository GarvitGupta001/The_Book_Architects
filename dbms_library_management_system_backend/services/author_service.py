...
def update_author(author_id, author_name=None, author_dob=None, author_origin=None, about=None):
    ...
    if author:
        if author_name:
            author.author_name = author_name.get('author_name',author.author_name)
        if author_dob:
            author.author_dob = author_dob.get('author_dob',author.author_dob)
        if author_origin:
            author.author_origin = author_origin.get('author_origin', author.author_origin)
        if about:
            author.about = about.get('about',author.about)
        db.session.commit()
        return author.to_dict()
    return author
